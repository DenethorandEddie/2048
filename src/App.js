...
/**
 * 2048+ — Minimal & Colorful
 * - Smooth animations
 * - Single-spawn after valid move
 * - Win/Game Over modals
 * - Light/Dark themes (default: LIGHT)
 * - Mobile friendly (no page scroll while swiping the board)
 * - State persisted (localStorage + cookie)
 */

/* ========== utils ========== */
const rnd = (n) => Math.floor(Math.random() * n);
const uid = (() => { let i = 1; return () => i++; })();
const emptyGrid = (n) => Array.from({ length: n }, () => Array(n).fill(null));
const cloneTiles = (tiles) => tiles.map((t) => ({ ...t }));
const toBoard = (tiles, n) => {
  const b = emptyGrid(n);
  for (const t of tiles) b[t.r][t.c] = t;
  return b;
};

function safeSpawn(tiles, n, bias = 0.9) {
  // Strong empty-check to prevent overlaps
...
function slideLine(line, dir) {
  ...
  for (const i of order) {
    ...
    while (true) {
      ...
      // merge only once per step & only equal values
      if (!locked.has(j) && line[j] && line[j].value === t.value) {
        ni = j;
      }
      break;
    }
    ...
    if (ni !== i) {
      if (line[ni] && line[ni].value === t.value && !locked.has(ni)) {
        ...
        // keep the "target" and bump it
        line[ni] = { ...line[ni], value: newVal, bump: true };
        line[i] = null;
        t.dead = true;              // source becomes dead (filtered out later)
        locked.add(ni);             // lock merged cell
        moved = true;
        gained += newVal;
      } else {
        line[ni] = t;
...
function move(tiles, n, dir) {
  ...
  // flatten board
  const next = [];
  for (let r = 0; r < n; r++) for (let c = 0; c < n; c++) if (b[r][c]) next.push(b[r][c]);

  // filter out merged-away "dead" tiles to avoid duplicates
...
const tileGradient = (v, theme) => {
  ...
  return `bg-gradient-to-br ${base[v] || base[2048]}`;
};

const fontSizeFor = (v) =>
  v < 128 ? "32px" : v < 1024 ? "34px" : v < 2048 ? "32px" : v < 4096 ? "28px" : "26px";

/* ========== component ========== */
export default function Game2048() {
  const [n, setN] = useState(4);
  const [bias] = useState(0.9);
  const [tiles, setTiles] = useState(() => safeSpawn(safeSpawn([], 4, bias), 4, bias));
  ...
  // helpers
  const reset = useCallback((nn = n) => {
    const start = safeSpawn(safeSpawn([], nn, bias), nn, bias);
    setN(nn);
    setTiles(start);
    setScore(0);
    setOver(false);
  ...
  const step = useCallback((dir) => {
    ...
    // after short delay, spawn exactly ONE tile in a free cell
    setTimeout(() => {
      setTiles((curr) => {
        const after = safeSpawn(curr, n, bias);
  ...
  }, [bias, best, board, busy, keptGoing, n, over, score, theme, tiles]);
  ...
  // touch (no page scroll inside board)
    ...
    <div className={`rounded-full border px-1 py-1 flex gap-1 ${pal.surface}`}>
      {[4, 5].map((s) => (
        <button
          key={s}
          onClick={() => reset(s)}
          className={`min-h-10 px-3 rounded-full text-sm transition font-medium ${n === s ? (isDark ? "bg-emerald-600 text-white shadow-sm" : "bg-emerald-500 text-white shadow-sm") : "hover:bg-white/10"}`}
        >
          {s}×{s}
        </button>
      ))}
  ...
  // NEW: Icon sits inside the knob, track shows faint hints; works perfectly on iOS Safari
  const ThemeToggle = () => {
    const TRACK_W = 96, KNOB_W = 52, PADDING = 4;
    const x = isDark ? 0 : TRACK_W - KNOB_W - PADDING * 2;
    const knobIcon = isDark ? "🌙" : "☀️";
    return (
      <button
        aria-pressed={isDark}
        title="Toggle theme"
        onClick={() => setTheme((t) => (t === "dark" ? "light" : "dark"))}
        className={`relative w-24 h-10 rounded-full border overflow-hidden ${pal.track}`}
      >
        {/* faint hints on track */}
        <div className="absolute inset-0 grid grid-cols-2 place-items-center text-xs opacity-60 select-none pointer-events-none">
          <span>🌙</span>
          <span>☀️</span>
        </div>
        {/* knob with centered icon */}
        <motion.div
          className={`absolute top-1 h-8 rounded-full shadow-md z-10 flex items-center justify-center text-base ${isDark ? "bg-zinc-800 text-white" : "bg-amber-300"}`}
          style={{ width: KNOB_W, left: PADDING }}
          animate={{ x }}
          transition={{ type: "spring", stiffness: 320, damping: 28 }}
        >
          <span className="select-none">{knobIcon}</span>
        </motion.div>
      </button>
    );
  };
  ...
  return (
    <div className={`min-h-screen ${pal.bg} p-4 sm:p-6`}>
      <div className="max-w-xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-4 sm:mb-6">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <button
              onClick={() => reset(n)}
              className="min-h-10 px-4 rounded-full text-sm font-semibold bg-gradient-to-tr from-emerald-500 to-emerald-400 text-white shadow hover:brightness-110"
            >
              New Game
            </button>
          </div>
            ...
            <AnimatePresence initial={false}>
              {tiles.map((t) => (
                <motion.div
                  ...
                  className={`absolute rounded-2xl font-black flex items-center justify-center shadow-lg border border-white/10 ${tileGradient(t.value, theme)}`}
                  style={{ width: cell, height: cell }}
                >
                  <motion.span
                    animate={t.bump ? { scale: [1, 1.18, 1] } : {}}
                    transition={{ duration: 0.18 }}
                    style={{ fontSize: fontSizeFor(t.value), textShadow: "0 1px 0 rgba(0,0,0,.15)" }}
                  >
                    {t.value}
                  </motion.span>
                </motion.div>
        ...
        <div className="mt-4 flex gap-2 justify-center">
          {["U", "L", "D", "R"].map((k) => (
            <button
              ...
              onClick={() => step(k)}
              disabled={busy}
              className={`min-h-10 px-4 rounded-full border ${pal.surface} disabled:opacity-50`}
            >
              {{ U: "↑", L: "←", D: "↓", R: "→" }[k]}
            </button>
          ))}
            ...
            <motion.div className="fixed inset-0 bg-black/70 flex items-center justify-center p-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              <motion.div initial={{ scale: 0.9, y: 10 }} animate={{ scale: 1, y: 0 }} className={`relative max-w-sm w-full rounded-3xl p-6 shadow-2xl ${pal.glass}`}>
                ...
                <div className="flex items-center gap-3 mb-3">
                  <div className={`h-11 w-11 rounded-2xl flex items-center justify-center text-2xl ${win ? "bg-emerald-500/20 border border-emerald-400/40" : "bg-rose-500/20 border border-rose-400/40"}`}>
                    {win ? "🏆" : "💀"}
                  </div>
                  <div>
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                  <button onClick={() => reset(n)} className="min-h-10 px-4 rounded-full bg-emerald-600 hover:bg-emerald-500 text-white">
                  ...
                  {win && (
                    <button
                      onClick={() => {
                        setWin(false);
                        setKeptGoing(true);
                        persist({ n, tiles, score, best, theme, keptGoing: true });
                      }}
                      className={`min-h-10 px-4 rounded-full border ${pal.surface}`}
                    >
                      Keep Going
                    </button>
                  )}
                  <button onClick={() => { setOver(false); setWin(false); }} className={`min-h-10 px-4 rounded-full border ${pal.surface}`}>
                    Close
                  </button>
                </div>
              </motion.div>
